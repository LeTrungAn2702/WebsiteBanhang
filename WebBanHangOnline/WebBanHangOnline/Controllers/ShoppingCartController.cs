        ...
        public ActionResult VnpayReturn()
        {
            if (Request.QueryString.Count > 0)
            {
                ...
                if (checkSignature)
                {
                    ...
                    {
                        itemOrder = db.Orders.FirstOrDefault(x => x.Code == orderCode);
                        if (itemOrder != null)
                        {
                            itemOrder.Status = 2;
                ...
                if (itemOrder != null)
                {
                    return RedirectToAction("CheckOutSuccess", new { id = itemOrder.Id });
                }
            }

            return RedirectToAction("Index", "ShoppingCart"); // fallback nếu không có dữ liệu
        }
        ...
        public ActionResult CheckOut(OrderViewModel req)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    cart.ClearCart();
                    code = new { Success = true, Code = req.TypePayment, Url = Url.Action("CheckOutSuccess", "ShoppingCart", new { id = order.Id }) };
